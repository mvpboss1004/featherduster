...
def rand_seeded_with_time_check(samples):
   ...
   if options_tmp == False:
      return False
   timestamps = list(range(options_tmp['base_timestamp']-86400,options_tmp['base_timestamp']+86400))

   prng_outputs = [seed_and_generate_value(timestamp, options_tmp['lowest'], options_tmp['highest']) for timestamp in timestamps]
   converted_samples = [int(sample, options_tmp['base']) for sample in samples]
